...
class DefaultController extends Controller
{
    ...
    public function generarHashAction() {
        $em = $this->getDoctrine()->getEntityManager();
        $ur = $em->getRepository("LoogaresUsuarioBundle:Usuario");
        ...
        foreach($usuarios as $usuario) {
            ...
            //$em->flush();
            echo "UPDATE usuarios SET hash_confirmacion = '".$hashConfirmacion."' WHERE id = ".$usuario->getId()."<br>";
        }
        echo sizeof($usuarios);
        echo "</pre>";

        return new Response('');
    }

    public function generarHash2Action() {
        $em = $this->getDoctrine()->getEntityManager();
        $ur = $em->getRepository("LoogaresUsuarioBundle:Usuario");
